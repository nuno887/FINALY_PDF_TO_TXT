...
CONFIG = {
    "input_dir": "input",
    "output_dir": "output",
    "pdf_filename": None,   # set to a filename to process only that file; None = process all in input/
...
def process_pdf_file(pdf_path: Path, output_root: Path, ignore_top_fraction: float, skip_last: bool,
    ...
    for i in range(0, effective_last + 1):
        ...
        export_single_page_pdf(doc, i, out_dir / f"page-{i+1:03d}.pdf")
    ...
    (out_dir / "completo.txt").write_text("\n\n".join(all_text), encoding="utf-8")
...
def main():
    ...
    if CONFIG.get("pdf_filename"):
        p = input_dir / CONFIG["pdf_filename"]
        if p.exists() and p.suffix.lower() == ".pdf":
            process_pdf_file(p, output_root, CONFIG.get("ignore_top_percent", 0.10),
                             CONFIG.get("skip_last_page", True), CONFIG.get("dpi", 300),
                             CONFIG.get("ocr_lang", "por"))
        else:
            print(f"File not found: {p.as_posix()}")
        return
    pdfs = sorted(input_dir.glob("*.pdf"))
    if not pdfs:
        print(f"No PDFs found in {input_dir.as_posix()}")
        return
    for p in pdfs:
        process_pdf_file(p, output_root, CONFIG.get("ignore_top_percent", 0.10),
                         CONFIG.get("skip_last_page", True), CONFIG.get("dpi", 300),
